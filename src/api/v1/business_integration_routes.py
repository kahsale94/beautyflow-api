...
from src.schemas import BusinessIntegrationResponse
...
@router.get("/{integration_id}", response_model=list[BusinessIntegrationResponse])
...
def create_business_integration(integration_id: int, business_id: BusinessScopeDep, service: BusinessIntegrationServiceDep, super_admin: SuperAdminDep):
    try:
        return service.create_business_integration(business_id, integration_id)
...
@router.put("/{integration_id}/rotate-key", status_code=204)
