...
from src.services.appointment_service import (AppointmentAlreadyCanceledError, AppointmentAlreadyCompletedError, AppointmentNotFoundError, AppointmentTimeConflictError,
    ClientNotFoundError, DatetimeFormatError, ProfessionalNotAvailableError, ProfessionalServiceMismatchError, ServiceNotAvailableError)
...
def _parse_calendar_datetime(value: str, timezone_name: str) -> datetime:
    ...
    if parsed.tzinfo is None:
        return parsed.replace(tzinfo=__import__("zoneinfo").ZoneInfo(timezone_name))
    return parsed
...
@router.get("/")
def calendar_page(request: Request, client_service: ClientServiceDep, professional_service: ProfessionalServiceDep, service_service: ServiceServiceDep, session: AdminSessionDep):
    return render(
        request,
...
def calendar_events(start: str, end: str, appointment_service: AppointmentServiceDep, business_service: BusinessServiceDep, client_service: ClientServiceDep,
    ...
    start_dt = _parse_calendar_datetime(start, business.timezone)
    end_dt = _parse_calendar_datetime(end, business.timezone)
...
def appointment_details_fragment(appointment_id: int, request: Request, appointment_service: AppointmentServiceDep, client_service: ClientServiceDep,
    professional_service: ProfessionalServiceDep, service_service: ServiceServiceDep, session: AdminSessionDep):
...
async def create_appointment_action(request: Request, appointment_service: AppointmentServiceDep, business_service: BusinessServiceDep, session: AdminSessionDep):
    ...
    try:
        ...
        data = AppointmentCreate(
            ...
            start_datetime=local_datetime_from_form(start_value, business.timezone),
        )
