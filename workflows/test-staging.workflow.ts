        ...
        formFields: {
            values: [
                ...
                },
                {
                    ...
                    fieldOptions: {
                        values: [
                            ...
                            {
                                option: 'main-staging | entrada CoverCut normalizada',
                            },
                            {
                                option: 'main-staging | Evolution fromMe ignorado',
                            },
                            {
                ...
                },
                {
                    ...
                    placeholder: 'Opcional. Ex: 5511974402483@s.whatsapp.net',
                },
                {
                    fieldLabel: 'Connection key override',
                    placeholder: 'Opcional. Ex: covercut:<phone-number-id> ou evolution:sale_instance',
                },
                {
...
const defaultPrivateJid = '5511974402483@s.whatsapp.net';
...
const connectionKeyOverride = field('connection_key_override', 'Connection key override');
const messageTextOverride = field('message_text_override', 'Mensagem override');

const remoteJid = remoteJidOverride || defaultPrivateJid;
...
const connectionKey = connectionKeyOverride || 'evolution:sale_instance';
...
function covercutBody(messageText) {
  const now = Date.now();
  return {
    schema_version: 1,
    provider: 'covercut',
    ...
    message: {
      id: 'TEST-COVERCUT-MSG-' + now,
      type: 'text',
      text: messageOverride || messageText,
    },
  };
}
...
function baseCase(config) {
  ...
  return {
    ...
    formOverrides: {
      ...
      connection_key_override: connectionKeyOverride,
...
const cases = {
  'main-staging | disponibilidade': baseCase({
    ...
    path: 'Webhook Evolution -> contexto de empresa/cliente -> classificador -> ferramenta de disponibilidades -> resposta',
    expectedResult: 'Retornar mensagem de confirmacao de horario, sem criar agendamento ainda.',
    dataImpact: 'Pode atualizar buffers e memoria Redis do atendimento de staging e enviar mensagem pelo gateway WhatsApp.',
  ...
  'main-staging | confirmar agendamento': baseCase({
    ...
    path: 'Webhook Evolution -> estado/contexto -> agente -> ferramenta de agendamentos -> resposta',
    expectedResult: 'Criar/confirmar um agendamento em staging e enviar resposta de confirmacao.',
    dataImpact: 'Pode criar appointment, evento de calendario, email de confirmacao, memoria Redis e mensagem pelo gateway WhatsApp.',
  ...
  'main-staging | entrada CoverCut normalizada': baseCase({
    target: 'main',
    routeIndex: 0,
    workflowName: 'main-staging',
    scenarioLabel: 'Entrada normalizada encaminhada pelo webhook CoverCut do backend',
    path: 'Webhook CoverCut normalizado -> data handler -> contexto -> classificador -> resposta',
    expectedResult: 'Processar a mensagem normalizada com isolamento por connection_key e responder pelo gateway WhatsApp.',
    dataImpact: 'Pode atualizar buffers e memoria Redis do atendimento e enviar mensagem pelo provedor associado a connection_key.',
    evidenceHint: 'data handler deve ler body.contact, body.message e body.connection_key; o envio deve usar /whatsapp/messages.',
    probableCauseHint: 'Se a autenticacao falhar, informe uma Connection key override CoverCut cadastrada no staging.',
    mainWebhook: {
      body: covercutBody('quero saber os horarios disponiveis amanha'),
    },
  }),

  'main-staging | Evolution fromMe ignorado': baseCase({
    target: 'main',
    routeIndex: 0,
    workflowName: 'main-staging',
    scenarioLabel: 'Mensagem Evolution enviada pelo proprio numero deve ser ignorada',
    path: 'Webhook Evolution -> data handler -> filtro fromMe -> fim sem resposta',
    expectedResult: 'Retornar HTTP 200 sem seguir para atendimento, memoria ou envio de mensagem.',
    dataImpact: 'Nao deve criar dados de negocio, memoria Redis nem enviar mensagem.',
    evidenceHint: 'A execucao deve parar no filtro fromMe para o payload com fromMe=true.',
    mainWebhook: {
      body: evolutionBody('teste fromMe staging', {
        fromMe: true,
  ...
  'businesses-staging | contexto por telefone': baseCase({
    ...
    dataImpact: 'Pode ler/gravar cache Redis beautyflow_bot.<connection_key>.<phone>.business_context.',
  ...
  'clients-staging | coletar nome pendente': baseCase({
    ...
    dataImpact: 'Pode atualizar cliente, pending state, memoria Redis e enviar mensagem pelo gateway WhatsApp.',
  ...
  'pending state-staging | voltar para coleta de nome': baseCase({
    ...
    dataImpact: 'Pode atualizar pending state, cliente, memoria Redis e enviar mensagem pelo gateway WhatsApp.',
  ...
  'error-staging | reportar erro interno': baseCase({
    ...
    dataImpact: 'Pode enviar notificacao pelo gateway WhatsApp/email conforme politica de erro.',
...
if (scenario === 'main-staging | entrada CoverCut normalizada' && !connectionKeyOverride.startsWith('covercut:')) {
  testCase.warnings.push('Informe uma Connection key override no formato covercut:<phone-number-id> para validar envio real pela CoverCut.');
}
...
return [{ json: testCase }];`,
            ...
            parameters: [
                {
                    ...
                    value: "={{ $('prepare test case').first().json.workflowInputs?.api?.connection_key || $('prepare test case').first().json.mainWebhook?.body?.connection_key || ($('prepare test case').first().json.mainWebhook?.body?.instance ? 'evolution:' + $('prepare test case').first().json.mainWebhook.body.instance : '') }}",
                },
            ],
        },
        options: {},
    ...
    ClearMainStagingChatBuffer = {
        operation: 'delete',
        key: "={{ $('prepare test case').first().json.cleanupKeys[0] }}",
    };
    ...
    ClearMainStagingChatMemory = {
        operation: 'delete',
        key: "={{ $('prepare test case').first().json.cleanupKeys[1] }}",
    };
    ...
    ClearMainStagingState = {
        operation: 'delete',
        key: "={{ $('prepare test case').first().json.cleanupKeys[2] }}",
    };
    ...
    ClearMainStagingOutsideHoursContext = {
        operation: 'delete',
        key: "={{ $('prepare test case').first().json.cleanupKeys[3] }}",
    };
    ...
    @node({
        id: '784b1e68-77e3-4f80-9a84-2f857f4e61d4',
        name: 'clear main-staging conversation meta',
        type: 'n8n-nodes-base.redis',
        version: 1,
        position: [-480, -544],
        credentials: { redis: { id: 'yq1GIl0nbdK5QpYm', name: 'beautyflow test' } },
    ...
    ClearMainStagingConversationMeta = {
        operation: 'delete',
        key: "={{ $('prepare test case').first().json.cleanupKeys[5] }}",
    };
...
- main-staging: jornadas reais via webhook multi-provider (Evolution ou CoverCut normalizado).
