                ...
                },
                {
                    ...
                    fieldOptions: {
                        values: [
                            ...
                            {
                                option: 'main-staging | privado ignorado',
                            },
                            {
                                option: 'main-staging | grupo fromMe',
                            },
                            {
...
const defaultPrivateJid = defaultGroupParticipantAlt;
...
function baseCase(config) {
  ...
  const remoteJidForCleanup = mainWebhookKey.remoteJid;
  const conversationJidForCleanup = String(remoteJidForCleanup || '').endsWith('@g.us')
    ? (mainWebhookKey.participantAlt || mainWebhookKey.participant || remoteJidForCleanup)
    : remoteJidForCleanup;
  const evoInstanceForCleanup = config.mainWebhook?.body?.instance || 'default';
  const redisConversationPrefix = 'beautyflow_bot.' + evoInstanceForCleanup + '.' + conversationJidForCleanup;
...
const cases = {
  ...
  'main-staging | privado ignorado': baseCase({
    target: 'main',
    routeIndex: 0,
    workflowName: 'main-staging',
    scenarioLabel: 'Mensagem privada ignorada pelo filtro de grupos',
    path: 'Webhook Evolution -> data handler -> filter group -> fim sem resposta',
    expectedResult: 'Retornar HTTP 200 do webhook sem seguir para atendimento, memoria ou envio Evolution.',
    dataImpact: 'Nao deve criar dados de negocio, memoria Redis nem enviar mensagem Evolution.',
    evidenceHint: 'Execucao deve parar no node filter group para payload privado.',
    mainWebhook: {
      body: evolutionBody('mensagem privada staging', {
        remoteJid: defaultPrivateJid,
      }),
    },
  }),

  'main-staging | grupo fromMe': baseCase({
    target: 'main',
    routeIndex: 0,
    workflowName: 'main-staging',
    scenarioLabel: 'Mensagem enviada pelo proprio numero dentro do grupo',
    path: 'Webhook Evolution -> data handler -> filter group -> atendimento -> resposta no grupo',
    expectedResult: 'Processar a mensagem de grupo mesmo com fromMe=true, usando participantAlt como cliente e respondendo no grupo.',
    dataImpact: 'Pode atualizar buffers e memoria Redis do atendimento de staging e enviar mensagem pela Evolution.',
    evidenceHint: 'client.remote_jid deve vir do participantAlt e send response deve usar o remoteJid do grupo.',
    mainWebhook: {
      body: (() => {
        const body = evolutionBody('teste fromMe grupo staging', {
          fromMe: true,
          participant: '142983974355178@lid',
          participantAlt: '5511991549118@s.whatsapp.net',
        });
        body.instance = 'beautyflow-barbearia-kaiky-1';
        return body;
      })(),
    },
  }),
  ...
  'businesses-staging | contexto por telefone': baseCase({
    ...
    dataImpact: 'Pode ler/gravar cache Redis beautyflow_bot.<instance>.<phone>.business_context.',
...
return [{ json: testCase }];`,
    ...
    ClearMainStagingChatBuffer = {
        operation: 'delete',
        key: `={{ (() => {
  const body = $('prepare test case').first().json.mainWebhook.body || {};
  const key = body.data?.key || {};
  const chatJid = String(key.remoteJid || '');
  const conversationJid = chatJid.endsWith('@g.us')
    ? String(key.participantAlt || key.participant || chatJid)
    : chatJid;
  return \`beautyflow_bot.\${body.instance || 'default'}.\${conversationJid}.chat_buffer\`;
})() }}`,
    };
  ...
  const conversationJid = chatJid.endsWith('@g.us')
    ? String(key.participantAlt || key.participant || chatJid)
    : chatJid;
  return \`beautyflow_bot.\${body.instance || 'default'}.\${conversationJid}.chat_memory\`;
})() }}`,
    };
  ...
  const conversationJid = chatJid.endsWith('@g.us')
    ? String(key.participantAlt || key.participant || chatJid)
    : chatJid;
  return \`beautyflow_bot.\${body.instance || 'default'}.\${conversationJid}.state\`;
})() }}`,
    };
  ...
  const conversationJid = chatJid.endsWith('@g.us')
    ? String(key.participantAlt || key.participant || chatJid)
    : chatJid;
  return \`beautyflow_bot.\${body.instance || 'default'}.\${conversationJid}.outside_hours_context\`;
})() }}`,
    };
  ...
  const conversationJid = chatJid.endsWith('@g.us')
    ? String(key.participantAlt || key.participant || chatJid)
    : chatJid;
  return \`beautyflow_bot.\${body.instance || 'default'}.\${conversationJid}.personal_block\`;
})() }}`,
    };
  ...
  const conversationJid = chatJid.endsWith('@g.us')
    ? String(key.participantAlt || key.participant || chatJid)
    : chatJid;
  return \`beautyflow_bot.\${body.instance || 'default'}.\${conversationJid}.conversation_meta\`;
})() }}`,
    };
